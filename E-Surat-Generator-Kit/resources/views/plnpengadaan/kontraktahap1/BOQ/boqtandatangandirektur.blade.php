...
@section('title', 'Tanda Tangan Manager')
            ...
            <div class="col-12">
                ...
                <div class="card">
                    <div class="card-header">
                        Sebagai seorang vendor, dengan menandatangani dokumen ini, Anda menyatakan persetujuan terhadap isi dokumen dan mengkonfirmasi bahwa dokumen tersebut telah disetujui. Dokumen ini akan dikirimkan ke pihak PLN untuk melanjutkan pelaksanaan kontrak.
                        <style>
                            #signature-canvas {
...
@section('javascript')
    ...
    <script>
        ...
        function convertToBase64() {
            ...
            xhr.open('POST', "{{ route('simpantandatanganboq') }}");
            xhr.setRequestHeader('X-CSRF-TOKEN', '{{ csrf_token() }}');
            xhr.onload = function() {
                if (xhr.status === 200) {
                    window.location.href = "{{ route('vendor.kontrakkerja.detail', ['id' => $id]) }}";
                    console.log('Tanda Tangan berhasil diupload');
                } else {
                    var response = JSON.parse(xhr.responseText);
                    var errorMessage = response.error_message; // Ubah "error_message" sesuai dengan properti yang sesuai dalam respons JSON
