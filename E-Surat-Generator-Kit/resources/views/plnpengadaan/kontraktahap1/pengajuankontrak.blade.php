                            ...
                            <tbody>
                                ...
                                @foreach ($kontrak as $item)
                                    <tr>
                                        ...
                                        <td>{{ date('d/m/Y', strtotime($item->tanggal_pekerjaan)) . ' - ' . date('d/m/Y', strtotime($item->tanggal_akhir_pekerjaan))}}</td>
                                        <td>{{ $item->status }}</td>
                                        <td>   <a href="{{route('pengajuankontrak.detail',['id'=>$item->id_kontrakkerja])}}" class="btn btn-warning">Dokumen Lanjutan</a></td>
                                        <td> <a href="{{ route('pengajuankontrak.edit', ['id' => $item->id_kontrakkerja]) }}"
                                                class="btn btn-primary">Edit</a>
                                             
                                            <form action="{{ route('pengajuankontrak.destroy', ['id' => $item->id_kontrakkerja]) }}"
